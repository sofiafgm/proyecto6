...
router.post('/new', async (req, res) => {
  ...
  db.run(
    ...
    function (err) {
      ...
      res.status(201).json({ mensaje: 'Usuario registrado', id: this.lastID, rol: rolFinal } );
    }
  );
});
...
router.post('/login', (req, res) => {
  ...
  db.get(`SELECT * FROM usuarios WHERE email = ?`, [email], async (err, user) => {
    ...
    res.json({ mensaje: 'Login exitoso', token, usuario: user.nombre, rol: user.rol, user_id: user.id});
  });
});
